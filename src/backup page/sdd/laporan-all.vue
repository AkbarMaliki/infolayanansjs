<template>
  <div class="font-times">
    <!-- FORM -->
      <p class="text-center text-uppercase pb-3">sdd_laporan</p>
      <hr class="style2">
    ...
    <div style="overflow-x:auto;width:100%;" v-if="ready">
      ...
      <div class="row">
          <div class="col-12">
              <p>Pilih Technical Support untuk mencetak laporannya : </p>
              <select class='form-control' v-model="userid" @input="prints=true">
                <option v-for="(item, index) in users" :key="index+'user'" :value="item.id">{{item.technical_support}}</option>
              </select>
               <div class="row">
                    <div class="col-6  text-center order-2">
                        <br>

                        <a v-show="prints" :href="`https://infolayanans.space/api/mysql/sdd/request.php?data=select wilayah,cabang,nama_requestor,deksripsi_request,tanggal_request,waktu_request,waktu_mulai_pengerjaan,severity_level,tanggal_penyelesaian,waktu_penyelesaian,sifat_problem,technical_support,deskripsi_support,durasi_jam,status_akhir from sdd_laporan where type='REQUEST'  AND tanggal_request LIKE '${this.tanggals}%' AND id_user='${this.userid}' order by tanggal_request&database=infolay3_test`" class="text-green btn btn-sm btn-dark "><span class="typcn typcn-chart-bar"></span> Request</a>
                    
                    </div>
                    <div class="col-6  text-center order-3">
                        <br>
                        <a v-show="prints" :href="`https://infolayanans.space/api/mysql/sdd/incident.php?data=select wilayah,cabang,nama_requestor,deksripsi_request,tanggal_request,waktu_request,waktu_mulai_pengerjaan,severity_level,tanggal_penyelesaian,waktu_penyelesaian,sifat_problem,technical_support,deskripsi_support,durasi_jam,status_akhir from sdd_laporan where type='INCIDENT'  AND tanggal_request LIKE '${this.tanggals}%' AND id_user='${this.userid}' order by tanggal_request&database=infolay3_test`" class="text-green btn btn-sm btn-dark "><span class="typcn typcn-chart-line"></span> Incident</a>
                    </div>
...
export default {
  ...
  watch: {
    ...
    let fd2 = new FormData()
      ...
      fd3.append('data', `select * from sdd_tbuser `)//database setting di server
