<template>
  <div class="font-times px-4 py-4">
    <!-- FORM -->
      <button type="button" @click="nexts" class="btn btn-sm btn-dark float-right ">Next</button>
      <br>
      <br>
      <p class="text-center text-uppercase pb-3">List Cabang</p>
     <p>Tambahkan List Cabang anda : </p>
      <br>
      <div class="row">
    ...
    >
      <!-- FORM --> 
          <tr class="d-none">
              <td class='text-uppercase text-sm'>id_cabang</td>
              <td>&nbsp;:&nbsp;</td>
          ...
          <tr>
              <td class='text-uppercase text-sm'>Nama Cabang</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                   <div class="tips">
                        <div class="sm-form">
                        <input
                            ...
                            placeholder="cabang"
                            id="cabang"
                            name="isi nama cabang ..."
                            class="form-control form-control-sm"
                            v-model="vdata.cabang"
                    required
                    
                        >
                        </div>
                      <span class="tipstextT">Contoh : KC Banjarbaru, KK Martapura</span>
                  </div>
              </td>
            </tr>
            <!-- FORM -->
      ...
      ><span class="typcn typcn-edit"></span> {{aksi == 'insert' ? 'Tambah' : aksi == 'update' ? 'update' : 'remove'}}</button>
      </div>
      <!-- SUBMIT -->
    </form>
     <hr class="style13" />
...
export default {
  ...
  methods: {
      nexts(){
          let fd = new FormData()
          fd.append('data', `update sdd_tbuser set reset='1' where id='${this.$store.state.users.id}'`)//database setting di server
          ...
          , {
              ...
              console.log(res)
              alert('Selamat Menggunakan Aplikasi !')
              let fd = new FormData()
            fd.append('username', this.$store.state.users.username)
            fd.append('password', this.$store.state.users.password)
            axios
      ...
      fd.append("data", `Insert into sdd_cabang (id_user,cabang) VALUES ('${this.$store.state.users.id}','${this.vdata.cabang}')`);
      fd.append("database", "infolay3_test");
      ...
      fd.append("data", `UPDATE sdd_cabang SET cabang='${this.vdata.cabang}' Where id_cabang = '${this.vdata.id_cabang}'`);
      fd.append("database", "infolay3_test");
      ...
      fd.append("data", `DELETE FROM sdd_cabang WHERE  id_cabang =  '${this.vdata.id_cabang}'`);
      fd.append("database", "infolay3_test");
     ...
     refreshData() {
      let fd2 = new FormData();
      fd2.append("data", `select id_cabang,id_user,cabang from sdd_cabang where id_user='${this.$store.state.users.id}'`); //if empty = *
      fd2.append('table', 'sdd_cabang')
