...
export default {
  ...
  methods: {
    ...
    getData() {
        db.collection(`${this.vdata.table}`).get().then(res=>{
            let arr=[]
            res.forEach(e=>{
    ...
    formula() {
      let that = this
      this.order = this.order.sort((a,b) => (a.no > b.no) ? 1 : ((b.no > a.no) ? -1 : 0)); 
      let obj = []
      this.datanya.forEach((e,index)=>{
        obj[index]=that.order[index].val
      })
      this.datanya=obj
  ...
  methods: {
    ...
    `;
      let methods = `insert(){
          ...
          db.collection("${this.vdata.table}").doc(this.pilih.ids).set(this.vdata,{merge:true})
      },remove(){
          if(confirm('Apakah yakin menghapus data?')){
              db.collection("${this.vdata.table}").doc(this.pilih.ids).delete()
          }
      }
      ,ambil(item, index) {
      window.scrollTo(0, 0);
