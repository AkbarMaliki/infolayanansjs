<template>
  <div class="font-times">
    <!-- FORM -->
      <p class="text-center text-uppercase pb-3">sekolah_absensi_siswa</p>
    ...
    >
      <!-- FORM --> 
          ...
          <tr>
              ...
              <td style="width:90%;padding-bottom:5px;">
                <select class='form-control'  required v-model="vdata.id_pelajaran">
                    <option v-for="(item, index) in pelajarans" :key="index+'siswas2'" :value="item.id_pelajaran">{{item.nama_pelajaran}}</option>
                  </select>
              </td>
            </tr>
          ...
          <tr>
              <td class='text-uppercase text-sm'>tanggal</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
                    ...
                    name="tanggal"
                    class="form-control form-control-sm"
                     required v-model="vdata.tanggal"
          ...
          <tr>
              <td class='text-uppercase text-sm'>hadir</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
               <input type="radio" id="one" value="hadir"  required v-model="vdata.hadir">
                <label for="one">Hadir</label>
                <br>
                <input type="radio" id="two" value="tidak hadir"  required v-model="vdata.hadir">
               <label for="two">Tidak Hadir</label>
              </td>
            </tr>
            <!-- FORM -->
      <!-- SUBMIT -->
...
export default {
  ...
  data() {
    return {
      ...
      err: false,
      checkedItem:[],
      less: [,'_id','id_siswa','id_kelas','id_pelajaran'],
      // modal
      modal: false,
      modalAction: "preview",
      // pagination
  ...
  methods: {
      ...
      fd.append("data", `Insert into sekolah_absensi_siswa (id_siswa,id_pelajaran,id_kelas,tanggal,hadir) VALUES ('${this.vdata.id_siswa}','${this.vdata.id_pelajaran}','${this.vdata.id_kelas}','${this.vdata.tanggal}','${this.vdata.hadir}')`);
      ...
      fd.append("data", `UPDATE sekolah_absensi_siswa SET id_siswa='${this.vdata.id_siswa}',id_pelajaran='${this.vdata.id_pelajaran}',id_kelas='${this.vdata.id_kelas}',tanggal='${this.vdata.tanggal}',hadir='${this.vdata.hadir}' Where id = '${this.vdata.id}'`);
      fd.append("database", "infolay3_test");
      ...
      fd.append("data", `DELETE FROM sekolah_absensi_siswa WHERE  id =  '${this.vdata.id}'`);
      fd.append("database", "infolay3_test");
     ...
     refreshData() {
      ...
      fd2.append("data", "select tb1.id,tb1.id_siswa,tb2.nama_siswa,tb1.id_pelajaran,tb3.nama_pelajaran,tb1.id_kelas,tb4.nama_kelas,tb1.tanggal,tb1.hadir from sekolah_absensi_siswa tb1 left join sekolah_data_siswa tb2 on tb2.id_siswa=tb1.id_siswa left join setup_pelajaran tb3 on tb3.id_pelajaran=tb1.id_pelajaran left join setup_kelas tb4 on tb4.id_kelas=tb1.id_kelas"); //if empty = *
      fd2.append('table', 'sekolah_absensi_siswa')
  ...
  watch: {
    ...
    td() {
      ...
      let fd = new FormData()
      fd.append('data', `select id_siswa,nama_siswa from sekolah_data_siswa`)//database setting di server
      fd.append('database','infolay3_test')
