<template>
  <div class="font-times">
    <!-- FORM -->
      ...
      <p class="text-center text-uppercase pb-3">sekolah_tugas</p>
    ...
    >
      <!-- FORM --> 
          <tr>
              <td class='text-uppercase text-sm'>id_tugas</td>
              <td>&nbsp;:&nbsp;</td>
          ...
          <tr>
              ...
              <td style="width:90%;padding-bottom:5px;">
               <select class='form-control' required v-model="vdata.id_guru">
                  <option v-for="(item, index) in sekolah_data_guru" :key="index+'sekolah_data_guru'" :value="item.id_guru">{{item.nama_guru}}</option>
                </select>
              </td>
            </tr>
          ...
          <tr>
              ...
              <td style="width:90%;padding-bottom:5px;">
                ...
                <option v-for="(item, index) in setup_pelajaran" :key="index+'setup_pelajaran'" :value="item.id_pelajaran">{{item.nama_pelajaran}}</option>
                </select>
              </td>
            </tr>
          ...
          <tr>
              ...
              <td style="width:90%;padding-bottom:5px;">
               <select class='form-control' required v-model="vdata.id_kelas">
                <option v-for="(item, index) in setup_kelas" :key="index+'setup_kelas'" :value="item.id_kelas">{{item.nama_kelas}}</option>
                </select>
              </td>
            </tr>
          ...
          <tr>
              <td class='text-uppercase text-sm'>file</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                  <div class="sm-form ">
                      <input type="text" id="files" name="files" class="form-control form-control-sm" placeholder="files" v-model="vdata.file" >
                  </div>
          ...
          <tr class="d-none">
              <td class='text-uppercase text-sm'>tanggal_awal</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
          ...
          <tr>
              <td class='text-uppercase text-sm'>tanggal_akhir</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
...
export default {
  ...
  data() {
    return {
      ...
      // Data datable
      datanya: [],
      sekolah_tugas:"",sekolah_data_guru:"",setup_pelajaran:"",setup_kelas:"",
      vdata: {
      ...
      err: false,
      checkedItem:[],
      less: [,'_id','id_guru','id_kelas','id_pelajaran','tanggal_awal'],
      // modal
      modal: false,
      modalAction: "preview",
      // pagination
  ...
  methods: {
    ...
    fd.append("id", this.$store.state.users.id_guru);
    axios
        ...
        this.vdata.file=res.data
        this.$forceUpdate()
        });
    },
      insert(){ let fd = new FormData();
      
      fd.append("data", `Insert into sekolah_tugas (id_guru,id_pelajaran,id_kelas,ket,file,tanggal_awal,tanggal_akhir) VALUES ('${this.vdata.id_guru}','${this.vdata.id_pelajaran}','${this.vdata.id_kelas}','${this.vdata.ket}','${this.vdata.file}','${this.vdata.tanggal_awal}','${this.vdata.tanggal_akhir}')`);
      ...
      fd.append("data", `UPDATE sekolah_tugas SET id_guru='${this.vdata.id_guru}',id_pelajaran='${this.vdata.id_pelajaran}',id_kelas='${this.vdata.id_kelas}',ket='${this.vdata.ket}',file='${this.vdata.file}',tanggal_awal='${this.vdata.tanggal_awal}',tanggal_akhir='${this.vdata.tanggal_akhir}' Where id_tugas = '${this.vdata.id_tugas}'`);
      fd.append("database", "infolay3_test");
      ...
      fd.append("data", `DELETE FROM sekolah_tugas WHERE  id_tugas =  '${this.vdata.id_tugas}'`);
      fd.append("database", "infolay3_test");
     ...
     refreshData() {
       console.log('walikelas',this.$store.state.wali_kelas)
      let fd2 = new FormData();
      fd2.append("data", `select tb1.id_tugas,tb2.nama_guru,tb1.id_guru,tb3.nama_pelajaran,tb1.id_pelajaran,tb4.nama_kelas,tb1.id_kelas,tb1.ket,tb1.file,tb1.tanggal_awal,tb1.tanggal_akhir from sekolah_tugas tb1 left join sekolah_data_guru tb2 on tb2.id_guru=tb1.id_guru left join setup_pelajaran tb3 on tb3.id_pelajaran=tb1.id_pelajaran left join setup_kelas tb4 on tb4.id_kelas=tb1.id_kelas where tb1.id_kelas='${this.$store.state.wali_kelas}'`); //if empty = *
      fd2.append('table', 'sekolah_tugas')
      ...
      axios
        ...
        .then(res => {
          ...
          setTimeout(() => {
            ...
            // this.datanya.filter(e=>{
            //   if(e.id_kelas==that.$store.state.walikelas){
            //     return e
            //   }
            // })
  ...
  watch: {
    ...
    td() {
      ...
      this.vdata.tanggal_awal=new Date()

let fd2 = new FormData()
fd2.append('data', `select id_guru,nama_guru from sekolah_data_guru`)//database setting di server
fd2.append('database','infolay3_test')
...
console.log(res)
this.sekolah_data_guru=res.data
this.$forceUpdate()
})
...
console.log(res)
this.setup_pelajaran=res.data
this.$forceUpdate()
})
                
let fd4 = new FormData()
fd4.append('data', `select id_kelas,nama_kelas from setup_kelas`)//database setting di server
