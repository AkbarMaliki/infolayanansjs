                ...
                <tr v-for="(item, index3) in td" :key="index3+'td'">
                  ...
                  <no-ssr>
                    <v-touch
                      ...
                      :key="indexs+'th2'"
                      v-html="item[item2] == 'available' ? 'Standby': item[item2]"
...
export default {
  ...
  methods: {
    ambil (item, index) {
      ...
      this.vdata = clone
      this.pilih = item
      this.$router.push('/driver/profile/'+clone.id)
    },
    clearInput () {
      this.formOn = true
    ...
    refreshData () {
      ...
      fd2.append('data', 'SELECT * FROM `tbuser`   where posisi="driver" ') // if empty = *
