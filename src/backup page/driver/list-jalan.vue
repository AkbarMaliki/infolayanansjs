                ...
                <tr v-for="(item, index3) in td" :key="index3+'td'">
                  ...
                  <no-ssr>
                    <v-touch
                      ...
                      :key="indexs+'th2'"
                      v-html="item[item2]=='3'?'Lagi Dijalan':item[item2]=='2'||item[item2]=='1'?'Ijin Keluar':item[item2]"
...
export default {
  ...
  methods: {
    ambil (item, index) {
      ...
      this.vdata = clone
      this.pilih = item
      this.$router.push('/driver/mobil/'+clone.id)
    },
    clearInput () {
      this.formOn = true
    ...
    refreshData () {
      ...
      fd2.append('data', 'SELECT tb1.kendaraan,tb2.nama as nama_driver,tb3.nama_mobil,tb1.penumpang,tb1.lokasi,tb1.jam_berangkat,tb1.status FROM driver_perjalanan tb1 left join tbuser tb2  on tb1.id_driver=tb2.id left join driver_mobil tb3 on tb3.id=tb1.id_mobil where tb1.status="1" OR tb1.status="2" OR tb1.status="3"') // if empty = *
