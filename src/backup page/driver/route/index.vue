  ...
  <div>
      <div class="rounded-br-lg"
        ...
        <div class="bg-white ml-2 mr-2 p-2 rounded-lg shadow-lg" style="margin-top:-50px;"  v-if="infos.jam_berangkat ? infos.jam_berangkat.length : 0 >0"> 
          <div v-if="infos.id_driver==this.$store.state.users.id">
            <div class="sm-form font-bold font-times">
                <label for="routerbaru">Route Baru</label>
                <input type="text" id="routerbaru" name="routerbaru" class="form-control form-control-sm" placeholder="route baru" v-model="routerbaru" >
            </div>
            <br>
            <div class="text-center">
              <button type="button" @click="updates" class="btn btn-sm btn-primary"><span class="typcn typcn-arrow-shuffle"></span> Update Route</button>
            </div>
            <br>
             <div class="text-center">
              <button type="button" @click="selesais" class="btn btn-sm btn-danger"><span class="typcn typcn-archive"></span> Selesai</button>
            </div>
          </div>
...
export default{
  ...
  methods: {
    ...
    selesais(){
      if (confirm("Yakin mengakhiri sesi perjalanan? ")) {
      ...
      , {
          ...
          fd2.append('data', `update tbuser set status='available', ket=' ' where id='${this.$store.state.users.id}'`)
          ...
          , {
            ...
            , {
              ...
              this.notifikasi(this.infos.id_penumpang,'/driver/req-driver','Sesi jalan telah selesai!');
              this.$store.dispatch('notification',{
              fcm : this.infos.id_penumpang,
              isi :'Sesi jalan telah selesai!'
              })
              this.mails(this.infos.id_penumpang,'NOTIFIKASI-JRBTN','Sesi jalan telah selesai!');
              setTimeout(() => {
    ...
    refreshRoute(){
      let fd = new FormData()
      fd.append('data', `select * from driver_route where id_perjalanan='${this.infos.id}'`)
      fd.append('database', ' infolay3_test')
      axios.post('https://infolayanans.space/api/mysql/auto.php', fd
      , {
          headers: {
