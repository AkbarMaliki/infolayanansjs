    ...
    <div class="bg-white p-2 mt-2  shadow-inner" style="margin-top:-50px;" v-if="klik">
      <div class="max-w-sm  overflow-hidden shadow-lg">
        ...
        <div class="px-6 pt-4">
            ...
            <tr class="text-grey-darker text-base" v-show="pilih.kendaraan=='kantor'">
                <td>Driver</td>
                <td>&nbsp;:&nbsp;</td>
                <td>{{pilih.nama}}</td>
            </tr>
            <tr class="text-grey-darker text-base" v-show="pilih.kendaraan=='kantor'">
                <td>Mobil</td>
                <td>&nbsp;:&nbsp;</td>
                <td>{{pilih.nama_mobil}}&nbsp;&nbsp;<span class="font-bold font-times">{{pilih.no_plate}}</span> </td>
            </tr>
      ...
      <div class="p-2">
        ...
        <button type="button" @click="approve" class="btn btn-sm btn-warning rounded-lg float-right font-bold font-times">Approve <span class="typcn typcn-arrow-right"></span></button>
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      err: false,
      checkedItem: [],
      less: ['_id','id','no_hp','gambar','status','id_driver','id_mobil','nama','nama_mobil','no_plate','id_penumpang','ttd'],
      // modal
      modal: false,
      modalAction: 'preview',
      // pagination
  ...
  methods: {
    batals(){
      ...
      let fd = new FormData()
      fd.append('data', `update tbuser set status='available' where id='${this.pilih.id_driver}' `)
      ...
      fd3.append('data', `update driver_mobil set status='available' where id='${this.pilih.id_mobil}' `)
      ...
      fd2.append('data', `delete from driver_perjalanan where id='${this.pilih.id}'`)
    ...
    approve(){
      console.log(this.datanya)
        if(confirm('Approve permintaan ? ')){
          alert('Permintaan sudah diterima, telah di notifikasikan ')
          this.notifikasi(this.datanya[0].id_penumpang,'/driver/req-driver','Permintaan Ijin telah di approve untuk '+this.datanya[0].penumpang);
          this.$store.dispatch('notification',{
          fcm : this.datanya[0].id_penumpang,
          isi :'Permintaan Ijin telah di approve untuk '+this.datanya[0].penumpang
          })
          this.notifikasi(this.datanya[0].id_driver,'/driver/route','Ijin Keluar untuk '+this.datanya[0].penumpang+' Tolong dibantu, Terima kasih!');
          this.$store.dispatch('notification',{
          fcm : this.datanya[0].id_driver,
          isi :'Ijin Keluar untuk '+this.datanya[0].penumpang+' Tolong dibantu, Terima kasih!'
          })
          // EMAIL
          this.mails(this.datanya[0].id_penumpang,'NOTIFIKASI-JRBTN','Permintaan Ijin telah di approve untuk '+this.datanya[0].penumpang);
          this.mails(this.datanya[0].id_driver,'NOTIFIKASI-JRBTN','Ijin Keluar untuk '+this.datanya[0].penumpang+' Tolong dibantu, Terima kasih!');
          this.simpans();
          setTimeout(() => {
            this.$router.push('/driver')
          }, 1000);
    ...
    mails(id,subjek,text){
     ...
     simpans(){
       let fd = new FormData()
       fd.append('data', `insert into driver_route (id_perjalanan,route,update_route) VALUES ('${this.pilih.id}','${this.pilih.lokasi}','${this.pilih.jam_berangkat}')`)
       fd.append('database', ' infolay3_test')
       axios.post('https://infolayanans.space/api/mysql/auto.php', fd
       , {
           headers: {
           ...
           this.$forceUpdate()
           let fd3 = new FormData()
           fd3.append('data', `select * from driver_route order by id desc`)
           fd3.append('database', ' infolay3_test')
           axios.post('https://infolayanans.space/api/mysql/auto.php', fd3
           , {
               headers: {
               ...
               let ids = res.data[0].id
                ...
                // let ids = 'this.pilih.id'
                fd4.append('data', `update driver_perjalanan set status='3',ids='${ids}', unit_heads='${this.$store.state.users.id}' where id='${this.pilih.id}' `)
          ...
          this.$forceUpdate()
           ...
           , {
               ...
               let ids = res.data[0].id
                let fd4 = new FormData()
                // let ids = 'this.pilih.id'
                fd4.append('data', `update driver_perjalanan set status='3',ids='${ids}' where id='${this.pilih.id}' `)
                fd4.append('database', ' infolay3_test')
                axios.post('https://infolayanans.space/api/mysql/auto.php', fd4
                , {
                    headers: {
    ...
    refreshData () {
      let fd2 = new FormData()
      fd2.append('data', `SELECT tb1.id_driver as id_driver,tb1.id_penumpang, tb1.id_mobil as id_mobil,tb1.penumpang,tb1.lokasi,tb1.status,tb1.id,tb1.tujuan,tb1.lokasi,tb1.keperluan,tb2.nama,tb2.no_hp,tb1.jam_berangkat,tb3.nama_mobil,tb3.no_plate,tb3.gambar,tb1.kendaraan,tb1.ttd FROM driver_perjalanan tb1 left join tbuser tb2 ON tb1.id_driver=tb2.id left join driver_mobil tb3 on tb1.id_mobil=tb3.id where tb1.status='2'`) // if empty = *
