  ...
  methods: {
    ...
    approve(){
      ...
      if(this.pilih.kendaraan=='kantor'){
          alert('approve proses dilanjutkan ke GA')
          this.simpans()
          let fd = new FormData()
          fd.append('data', `select * from tbuser where unit_kerja='2'`)
          fd.append('database', ' infolay3_test')
          axios.post('https://infolayanans.space/api/mysql/auto.php', fd
          , {
              headers: {
            ...
            res.data.filter(e=>{
              this.notifikasi(e.id,'/driver/list-request','Permintaan Approve Ijin Keluar oleh '+this.pilih.penumpang);
              this.$store.dispatch('notification',{
              fcm : e.id,
          ...
          fd2.append('data', `select * from tbuser where unit_kerja='2'`)
    ...
    simpans(){
      let fd = new FormData()
      fd.append('data', `update driver_perjalanan set unit_head='${this.$store.state.users.id}' where id='${this.pilih.id}' `)
      fd.append('database', ' infolay3_test')
      axios.post('https://infolayanans.space/api/mysql/auto.php', fd
      , {
          headers: {
