                ...
                <tr v-for="(item, index3) in td" :key="index3+'td'">
                  ...
                  <no-ssr>
                    <v-touch
                      ...
                      :class="{'text-green text-uppercase':item[item2]=='available'}"
...
export default {
  ...
  data () {
    return {
      ...
      err: false,
      checkedItem: [],
      less: ['id_driver','id_penumpang','ids','jam_update','id_mobil','status'],
      // modal
      modal: false,
      modalAction: 'preview',
      // pagination
  ...
  methods: {
    ambil (item, index) {
      ...
      this.vdata = clone
      this.pilih = item
      this.$router.push('/driver/laporan/route/'+clone.id)
    },
    clearInput () {
      this.formOn = true
    ...
    refreshData () {
      ...
      if(this.$store.state.users.unit_kerja==1 || this.$store.state.users.unit_kerja==2){
          fd2.append('data', `Select tb1.id,tb1.pilih,tb1.kendaraan,tb1.tujuan,tb1.keperluan,tb1.penumpang,tb1.tanggal,tb2.nama as driver,tb3.nama_mobil,tb3.no_plate FROM driver_perjalanan tb1 LEFT JOIN tbuser tb2 ON tb1.id_driver= tb2.id LEFT JOIN driver_mobil tb3 ON tb1.id_mobil= tb3.id`) // if empty = *
      }else{
          fd2.append('data', `Select tb1.id,tb1.pilih,tb1.kendaraan,tb1.tujuan,tb1.keperluan,tb1.penumpang,tb1.tanggal,tb2.nama as driver,tb3.nama_mobil,tb3.no_plate FROM driver_perjalanan tb1 LEFT JOIN tbuser tb2 ON tb1.id_driver= tb2.id LEFT JOIN driver_mobil tb3 ON tb1.id_mobil= tb3.id where id_penumpang="${this.$store.state.users.id}" OR id_driver="${this.$store.state.users.id}"`) // if empty = *
      }
      fd2.append('table', 'driver_perjalanan')
