      ...
      <div style="overflow-x:auto;width:100%;" v-if="ready">
        ...
        <div class="container flex flex-wrap mx-auto px-4 d-print-none">
          ...
          <!-- <jsontoexcel :data="td" class /> -->
              <a :href="`https://infolayanans.space/api/mysql/excel.php?data=Select tb1.id,tb1.pilih,tb1.kendaraan,tb1.tujuan,tb1.keperluan as rincian_kerja,tb1.lokasi,tb1.penumpang,tb1.tanggal,tb2.nama as driver,tb3.nama_mobil,tb3.no_plate FROM driver_perjalanan tb1 LEFT JOIN tbuser tb2 ON tb1.id_driver= tb2.id LEFT JOIN driver_mobil tb3 ON tb1.id_mobil= tb3.id where id_penumpang='${this.$store.state.users.id}' OR id_driver='${this.$store.state.users.id}'`">EXCEL</a>&nbsp;&nbsp;
...
export default {
  ...
  methods: {
    ambil (item, index) {
      ...
      this.vdata = clone
      this.pilih = item
      this.$router.push('/driver/laporan/'+clone.id)
    },
    clearInput () {
      this.formOn = true
    ...
    refreshData () {
      ...
      fd2.append('data', `Select tb1.id,tb1.pilih,tb1.kendaraan,tb1.tujuan,tb1.keperluan as rincian_kerja,tb1.lokasi,tb1.penumpang,tb1.tanggal,tb2.nama as driver,tb3.nama_mobil,tb3.no_plate FROM driver_perjalanan tb1 LEFT JOIN tbuser tb2 ON tb1.id_driver= tb2.id LEFT JOIN driver_mobil tb3 ON tb1.id_mobil= tb3.id where id_penumpang="${this.$store.state.users.id}" OR id_driver="${this.$store.state.users.id}"`) // if empty = *
