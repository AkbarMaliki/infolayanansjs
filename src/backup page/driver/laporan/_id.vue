  ...
  <div style="font-family:times new roman;">
    ...
    <div style='width:100%;'>
      <div class="row">
        ...
        style="margin-left:10px;position:absolute;left:20px;height:60px;"
        alt="no found"
      />
        </div>
        <div class="col-9">
      ...
      <br />
        <p style="position:absolute;right:10%;" class="font-bold">{{datanya.pilih}}</p>
      <br />
      <!--  -->
      <!-- TABLE -->
      <!--  -->
      <div class="pl-5">
      <tr v-for="(item,key, index) in datanya" :key="index" v-if="key!='pilih' && key!='ttd' && key!='no_plate' && key!='unit_kerja' && key!='unit_head'" style="margin-top:20px;">
        <td class="text-uppercase">{{key.replace('_',' ')}}</td>
        <td>&nbsp;:&nbsp;</td>
      ...
      <div class="row">
        ...
        <div class="text-center">
          <img :src="ttds3" class="kinoLightBox img-fluid " style="width:130px;height:130px;">
        </div>
        ...
        <br />
        <div class="text-center">
          <img :src="ttds2" class="kinoLightBox img-fluid " style="width:130px;height:130px;">
        </div>
        <p class="underline font-bold" style="font-size:19px;">{{ttd.Nama}}</p>
        </div>
        <div class="col-4">
        ...
        <br />
        <div class="text-center">
          <img :src="ttds1" class="kinoLightBox img-fluid " style="width:130px;height:130px;">
        </div>
        <p class="underline font-bold" style="font-size:19px;">{{ttd.Nama}}</p>
        </div>
      </div>
...
export default {
  ...
  methods: {
    refresh(){
      let fd = new FormData()
      fd.append('data', `Select tb1.penumpang,tb1.unit_kerja,tb1.unit_head,tb1.pilih,tb1.ttd,tb1.kendaraan,tb1.tujuan,tb1.keperluan as rincian_pekerjaan,tb1.lokasi as lokasi,tb1.tanggal,tb2.nama as driver,tb3.nama_mobil,tb3.no_plate FROM driver_perjalanan tb1 LEFT JOIN tbuser tb2 ON tb1.id_driver= tb2.id LEFT JOIN driver_mobil tb3 ON tb1.id_mobil= tb3.id where tb1.id='${this.$route.params.id}'`)
      ...
      , {
          ...
          this.datanya = res.data[0]
            ...
            fd2.append('data', `select * from tbuser_ttd where id_user='${this.datanya.unit_head}'`)
  ...
  mounted() {
    this.refresh()
    let fd = new FormData()
    fd.append('data', `select * from tbuser_ttd where id_user='${this.$store.state.users.id}'`)
    fd.append('database', ' infolay3_test')
    axios.post('https://infolayanans.space/api/mysql/auto.php', fd
    , {
        headers: {
    ...
    fd3.append('data', `select * from tbuser_ttd tb1 left join tbuser tb2 on tb2.id=tb1.id_user where tb2.unit_kerja='1'`)
