...
export default {
  ...
  data () {
    return {
      ...
      err: false,
      checkedItem: [],
      less: ['_id','id','no_hp','gambar','status','id_driver','id_mobil','nama','nama_mobil','no_plate','id_user'],
      // modal
      modal: false,
      modalAction: 'preview',
      // pagination
  ...
  methods: {
    ...
    approve(){
     ...
     simpans(){
       let fd = new FormData()
       fd.append('data', `update driver_lembur set status='3',disposisi1='${this.vdata.disposisi}' where id='${this.pilih.id}'`)
       fd.append('database', ' infolay3_test')
       axios.post('https://infolayanans.space/api/mysql/auto.php', fd
       , {
           headers: {
          ...
          this.notifikasi(this.pilih.id_user,'/driver/lembur/list-lembur','Permintaan Lembur telah di approve!');
          this.$store.dispatch('notification',{
            fcm : this.pilih.id_user,
            isi : 'Permintaan Lembur telah di approve!'
          })
          this.mails(this.pilih.id_user,'NOTIFIKASI-JRBTN','Permintaan Lembur telah di approve!');
          this.$router.push('/driver')
       }).catch(err=>{
        this.notifikasi(this.pilih.id_user,'/driver/lembur/list-lembur','Permintaan Lembur telah di approve!');
        this.$store.dispatch('notification',{
    ...
    refreshData () {
      ...
      fd2.append('data', `SELECT  tb2.nama,tb2.posisi,tb1.jenis_pekerjaan,tb1.jam_lembur,tb1.jam_lama,tb1.rincian_kerja,tb1.tanggal,tb1.status,tb1.id,tb1.id_user FROM driver_lembur tb1 LEFT JOIN tbuser tb2 ON tb1.id_user= tb2.id where  tb1.status='1' ORDER BY tb1.id_user ASC`) // if empty = *
