  ...
  <div>
    ...
    <div class="bg-white p-2 mt-2  shadow-inner" style="margin-top:-50px;" v-if="klik">
      <div class="max-w-sm  overflow-hidden shadow-lg">
        ...
        <div class="p-3">
          <p class="pl-2 font-bold ">Upload Laporan</p>
        </div>
        <div class="row" v-if="pilih.head==0">
          <div class="col-3"></div>
          <div class="col-6 text-center"><a :href="`https://wa.me/${ga}?text=Laporan Lembur Tanggal ${format(pilih.tanggal)}`" class="btn btn-sm btn-outline-success"><span class="typcn typcn-message-typing"></span> GA</a></div>
          <div class="col-3"></div>
        </div>
        <div class="row" v-else>
          <div class="col-6 text-center"><a :href="`https://wa.me/${dsm}?text=Laporan Lembur Tanggal ${format(pilih.tanggal)}`" class="btn btn-sm btn-outline-success"><span class="typcn typcn-message-typing"></span> DSM</a></div>
          <div class="col-6 text-center"><a :href="`https://wa.me/${dbm}?text=Laporan Lembur Tanggal ${format(pilih.tanggal)}`" class="btn btn-sm btn-outline-success"><span class="typcn typcn-message-typing"></span> DBM</a></div>
        </div>
        ...
        <div class="container flex flex-wrap mx-auto px-4 d-print-none">
          <div class="w-full sm:w-1/3">
            <div class="input-group mb-2">
              <a :href="`https://infolayanans.space/api/mysql/excel.php?data=SELECT  tb2.nama,tb2.posisi,tb1.jenis_pekerjaan,tb1.jam_lama,tb1.rincian_kerja,tb1.tanggal FROM driver_lembur tb1 LEFT JOIN tbuser tb2 ON tb1.id_user= tb2.id where  tb1.status='3' ORDER BY tb1.id_user ASC`" v-if="$store.state.users.unit_kerja=='1' || $store.state.users.unit_kerja=='2'">EXCEL</a>
              <a :href="`https://infolayanans.space/api/mysql/excel.php?data=SELECT  tb2.nama,tb2.posisi,tb1.jenis_pekerjaan,tb1.jam_lama,tb1.rincian_kerja,tb1.tanggal FROM driver_lembur tb1 LEFT JOIN tbuser tb2 ON tb1.id_user= tb2.id where  tb1.status='3' AND id_user='${$store.state.users.id}' ORDER BY tb1.id_user ASC`" v-else>EXCEL</a>
...
export default {
  ...
  data () {
    return {
      ...
      err: false,
      checkedItem: [],
      less: ['_id','id','no_hp','gambar','status','id_driver','id_mobil','nama','nama_mobil','no_plate','file','head'],
      // modal
      modal: false,
      modalAction: 'preview',
      // pagination
  ...
  methods: {
    ...
    onchangefile(e) {
        ...
        inputfile() {
          ...
          axios
            ...
            .then(res => {
              console.log(this.pilih.id)
              let fd2 = new FormData()
              fd2.append('data', `update driver_lembur_file set file='${res.data}' where id_lembur='${this.pilih.id}'`)
    ...
    refreshData () {
      let fd2 = new FormData()
      if(this.$store.state.users.unit_kerja==1 || this.$store.state.users.unit_kerja==2){
        fd2.append('data', `SELECT  tb2.nama,tb2.posisi,tb1.jenis_pekerjaan,tb1.jam_lembur,tb1.jam_lama,tb1.rincian_kerja,tb1.tanggal,tb3.file,tb1.status,tb1.id,tb1.head FROM driver_lembur tb1 LEFT JOIN tbuser tb2 ON tb1.id_user= tb2.id left join driver_lembur_file tb3 on tb3.id_lembur=tb1.id where  tb1.status='3' ORDER BY tb1.id_user ASC`) // if empty = *
      }else{
        fd2.append('data', `SELECT  tb2.nama,tb2.posisi,tb1.jenis_pekerjaan,tb1.jam_lembur,tb1.jam_lama,tb1.rincian_kerja,tb1.tanggal,tb1.status,tb3.file,tb1.id,tb1.head FROM driver_lembur tb1 LEFT JOIN tbuser tb2 ON tb1.id_user= tb2.id left join driver_lembur_file tb3 on tb3.id_lembur=tb1.id where  tb1.status='3' AND id_user='${this.$store.state.users.id}' ORDER BY tb1.id_user ASC`) // if empty = *
      }
  ...
  mounted () {
    let that = this
    let fd = new FormData()
    fd.append('data', `select * from tbuser_nohp`)//database setting di server
    axios.post('https://infolayanans.space/api/mysql/auto.php', fd
    , {
        headers: {
        Authorization: localStorage.getItem('auth._token.local')
        ...
        console.log('nohp',res)
        this.ga = res.data[0].ga
        this.dbm = res.data[0].dbm
        this.dsm = res.data[0].dsm
        this.$forceUpdate()
    })
    that.mediaQueries()
    this.$nextTick(() => {
