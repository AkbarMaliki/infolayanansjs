      ...
      <div class="max-w-sm  overflow-hidden shadow-lg">
        ...
        <div class="px-6 pt-4">
          <p class="text-grey-darker text-base">
            Karyawan : {{pilih.penumpang}}
          </p>
              <p class="text-grey-darker text-base">
            Tujuan : {{pilih.tujuan}}
          </p>
             <p class="text-grey-darker text-base">
            Rincian Pekerjaan : {{pilih.keperluan}}
          </p>
           <p class="text-grey-darker text-base">
            Lokasi Tujuan : {{pilih.lokasi}}
          </p>
        ...
        <div class="px-6 py-4">
          <span class="inline-block bg-grey-lighter rounded-full px-3 py-1 text-sm font-semibold text-grey-darker">Jam {{pilih.jam_berangkat}}</span>
        </div>
        <div class="p-2" v-show="pilih.kendaraan=='kantor'">
          <p>Driver : </p>
          <select class='form-control' v-model="drivers2">
             <option :value="item.id" v-for="(item, index) in drivers" :key="index+'drivers'">{{item.nama}}</option>
          </select>
        </div>
        <div class="p-2"  v-show="pilih.kendaraan=='kantor'">
          <p>Mobil : </p>
          <select class='form-control' v-model="mobils2">
          <option :value="item.id" v-for="(item, index) in mobils" :key="index+'mobils'">{{item.nama_mobil}}&nbsp;|&nbsp;{{item.no_plate}}</option>
          </select>
        </div>
        <div class="p-2" >
          <img :src="pilih.ttd" class="kinoLightBox img-fluid  " style="height:180px;">
        </div>
      </div>
      <div class="p-2">
        <button type="button" @click="klik=false" class="btn btn-sm btn-primary  "><span class="typcn typcn-arrow-left"></span> Kembali</button>
        <button type="button" @click="approve" class="btn btn-sm btn-success float-right ">Approve <span class="typcn typcn-arrow-right"></span></button>
      </div>
      <div class="text-center">
        <button type="button" @click="batals" class="btn btn-sm btn-danger">Batal <span class="typcn typcn-cancel"></span></button>
      </div>
    </div>
                ...
                <tr v-for="(item, index3) in td" :key="index3+'td'">
                  ...
                  <no-ssr>
                    <v-touch
                      ...
                      :key="indexs+'th2'"
                      v-html="item[item2]=='1'?'Request Mobil':item[item2]=='2'?'Minta Approve':item[item2]"
...
export default {
  ...
  data () {
    return {
      ...
      mobils2: '',
      checkedItem: [],
      less: ['_id','id','no_hp','gambar','status','nama','nama_mobil','no_plate','ttd'],
      // modal
      modal: false,
      modalAction: 'preview',
      // pagination
  ...
  methods: {
    batals(){
      let fd = new FormData()
      fd.append('data', `delete from driver_perjalanan where id=${this.pilih.id}`)
      ...
      , {
          ...
          alert('Permintaan ditolak!')
           ...
           fcm : this.pilih.id_penumpang,
           isi :'Permintaan anda ditolak atau dibatalkan '+this.pilih.penumpang
           })
           this.mails(this.pilih.id_penumpang,'NOTIFIKASI-JRBTN','Permintaan anda ditolak atau dibatalkan '+this.pilih.penumpang);
    ...
    approve(){
      console.log(this.drivers2)
      console.log(this.mobils2)
      console.log(this.pilih.kendaraan)
      console.log(this.pilih)
      if(confirm('Approve proses permintaan ?')){

      if(this.pilih.kendaraan=='kantor'){
        if(this.drivers2.length>0 && this.mobils2.length>0){
          alert('approve proses dilanjutkan ke Head')
          this.simpans()
          let fd = new FormData()
          fd.append('data', `select * from tbuser where unit_kerja='1'`)
          fd.append('database', ' infolay3_test')
          axios.post('https://infolayanans.space/api/mysql/auto.php', fd
          , {
              headers: {
            ...
            res.data.filter(e=>{
              this.notifikasi(e.id,'/driver/approve','Permintaan Approve Ijin Keluar oleh '+this.pilih.penumpang);
              this.$store.dispatch('notification',{
              fcm : e.id,
              isi :'Permintaan Approve Ijin Keluar oleh '+this.pilih.penumpang
              })
              this.mails(e.id,'NOTIFIKASI-JRBTN','Permintaan Approve Ijin Keluar oleh '+this.pilih.penumpang);
            })
            setTimeout(() => {
              this.$router.push('/driver')
            }, 2000);
          ...
          alert('Driver dan Mobil harus di isi!')
        }
      }else{
           this.simpans()
          alert('approve proses dilanjutkan ke Head')
          let fd2 = new FormData()
          fd2.append('data', `select * from tbuser where unit_kerja='1'`)
    ...
    simpans(){
      let fd = new FormData()
      fd.append('data', `update driver_perjalanan set id_driver='${this.drivers2}',id_mobil='${this.mobils2}', status='2' where id='${this.pilih.id}' `)
      fd.append('database', ' infolay3_test')
      axios.post('https://infolayanans.space/api/mysql/auto.php', fd
      , {
          headers: {
       ...
       let fd2 = new FormData()
      fd2.append('data', `update tbuser set status='Tidak Tersedia',ket='Bersama ${this.pilih.penumpang }' where id='${this.drivers2}' `)
      fd2.append('database', ' infolay3_test')
      axios.post('https://infolayanans.space/api/mysql/auto.php', fd2
      , {
          headers: {
      ...
      fd3.append('data', `update driver_mobil set status='Tidak Tersedia' where id='${this.mobils2}' `)
    ...
    refreshData () {
      let fd2 = new FormData()
      fd2.append('data', `SELECT tb1.penumpang,tb1.status,tb1.id,tb1.tujuan,tb1.lokasi,tb1.keperluan,tb2.nama,tb2.no_hp,tb1.jam_berangkat,tb3.nama_mobil,tb3.no_plate,tb3.gambar,tb1.kendaraan,tb1.ttd,tb1.id_penumpang,tb1.id_mobil FROM driver_perjalanan tb1 left join tbuser tb2 ON tb1.id_driver=tb2.id left join driver_mobil tb3 on tb1.id_mobil=tb3.id where tb1.status='1'`) // if empty = *
  ...
  mounted () {
    let that = this
    let fd = new FormData()
    fd.append('data', `select id,nama_mobil,no_plate from driver_mobil where status='available'`)
    fd.append('database', ' infolay3_test')
    axios.post('https://infolayanans.space/api/mysql/auto.php', fd
    , {
        headers: {
      ...
      let fd2 = new FormData()
    fd2.append('data', `select * from tbuser where status='available' AND posisi='driver'`)
    fd2.append('database', ' infolay3_test')
    axios.post('https://infolayanans.space/api/mysql/auto.php', fd2
    , {
        headers: {
