        ...
        >
          ...
          >
            <!-- <img
              ...
              style="height:30px;"
            /> -->
            <span class="pt-2 text-md pr-2 text-uppercase">HALAMAN </span>
            <span class="typcn typcn-thermometer"></span>
          </div>
          <!-- LEFT SECOND -->
          <div class="text-center" :style="{'background':bgleft2}">
            <br>
            <img src="@/static/driver/sdd.png" class="kinoLightBox img-fluid  " style="width:40%;">
            <br>
            <br>


            <p class="font-times font-bold">APLIKASI SEDERHANA INPUT LAPORAN</p>
            <br />
            <!-- MENU BUTTON -->
            <div
              class="bord hover:bg-red text-left pl-3 animated fadeInLeft"
              style="border-top:2px solid black;color:white;"
              @click="$router.push('/sdd')"
            >
            ...
            <div
              ...
              @click="muncul($event)"
            >
              <span class="typcn typcn-spanner"></span> Setting
              <span class="typcn typcn-arrow-down float-right"></span>
            </div>
            <ul class="isaktif pl-4 anim mb-3 text-white">
              <li  @click="$router.push('/sdd/set-user')"  class="bord hover:bg-red text-left pl-3 animated fadeInLeft">Profile</li>
              <li @click="$router.push('/sdd/set-cabang')" class="bord hover:bg-red text-left pl-3 animated fadeInLeft">List Cabang</li>
            </ul>
             <div
              class="bord hover:bg-red text-left pl-3 animated fadeInLeft"
              style="border-top:2px solid black;color:white;"
              v-show="$store.state.users.posisi=='koordinator'"
              @click="muncul($event)"
            >
              <span class="typcn typcn-spanner"></span> Koordinator
              <span class="typcn typcn-arrow-down float-right"></span>
            </div>
            <ul class="isaktif pl-4 anim mb-3 text-white">
              <li @click="$router.push('/sdd/laporan-kanwil')" class="bord hover:bg-red text-left pl-3 animated fadeInLeft">Laporan Kanwil</li>
            </ul>
             <div
              class="bord hover:bg-red text-left pl-3 animated fadeInLeft"
              style="border-top:2px solid black;color:white;"
              v-show="$store.state.users.posisi=='head'"
              @click="muncul($event)"
            >
              <span class="typcn typcn-spanner"></span> Head
              <span class="typcn typcn-arrow-down float-right"></span>
            </div>
              <div
              class="bord hover:bg-red text-left pl-3 animated fadeInLeft"
              style="border-top:2px solid black;color:white;"
              v-show="$store.state.users.posisi=='admin'"
              @click="muncul($event)"
            >
              <span class="typcn typcn-spanner"></span> Admin
              <span class="typcn typcn-arrow-down float-right"></span>
            </div>
            <ul class="isaktif pl-4 anim mb-3 text-white">
              <li @click="$router.push('/sdd/laporan-all')" class="bord hover:bg-red text-left pl-3 animated fadeInLeft">Laporan Semua</li>
              <li @click="$router.push('/sdd/list-user')" class="bord hover:bg-red text-left pl-3 animated fadeInLeft">List User</li>
            </ul>
            <!-- MENU 3 -->
          ...
          <div class="row">
            ...
            <div class="col-sm-12" style="padding:0;margin:0;">
              ...
              >
                ...
                <span class="text-white font-times pl-5 text-sm"></span>
                <template class>
       ...
       if (localStorage.getItem('auth._token.local')) {
          ...
          .then(res => {
              ...
              localStorage.setItem('users', JSON.stringify(res.data[0]))
                if(this.$store.state.users.reset=='3'){
                  this.$router.push('/sdd/register2')
                }else if(this.$store.state.users.reset=='0'){
                  this.$router.push('/sdd/reset')
                }
          }).catch(err=>{
              console.log(err)
          })
